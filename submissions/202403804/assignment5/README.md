...
* 모델 선택: mBART-large-cc25 모델을 채택했다. 이는 다양한 언어 쌍에 대해 사전 학습된 트랜스포머 기반의 Seq2Seq 모델로, NMT 태스크, 특히 한국어와 같은 저자원 언어 쌍의 학습 효율을 높이는 데 적합했다.

### 2.2. 데이터셋 분할 및 전처리

AI HUB 데이터셋을 활용하여 모델 학습에 필요한 데이터셋을 구성한했다.
...
* 전처리: `AutoTokenizer`를 사용하여 입력과 타겟 시퀀스를 최대 길이 50으로 제한한다. 소스 언어는 `"en_XX"`, 타겟 언어는 `"ko_KR"`로 설정했다.
...
* 평가 지표: 기계 번역의 표준 성능 지표인 BLEU Score (sacreBLEU)를 사용했다. BLEU Score는 생성된 번역과 참조 번역 간의 n-gram 일치도를 측정했다.
...
* 재현성: `inference.ipynb`를 통해 모델 로드, PDF 텍스트 전처리, 추론, 결과 출력이 모두 독립적으로 실행 가능했다. 
...
* 도메인 불일치(Domain Mismatch) 문제: Test Set의 BLEU Score(7.61)가 Validation Set(24.14) 대비 크게 낮게 측정되었다. 이는 Test Set으로 사용된 강의 자료 데이터가 훈련 데이터의 일반적인 도메인과 달라 발생하는 도메인 일반화 성능의 부족을 시사한다.
* 데이터 제약 및 과적합: 학습 데이터의 규모(20,000쌍)가 상대적으로 제한되어 있으며, Validation Loss(1.1770) 대비 Test Loss(2.4105)가 높아 Test Set에서 모델이 기대만큼 일반화되지 못한 것으로 판단한다.
