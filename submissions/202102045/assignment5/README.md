...
공개 다운로드 링크: Google Drive Model Download Link
...
문장 끊김 현상 (Truncation Issue)

현상: 긴 본문을 요약할 때, 설정된 최대 길이(max_length) 제한으로 인해 문장이 완벽하게 맺어지지 않고 중간에 끊기는 현상이 일부 관찰되었습니다.

Solution:

파라미터 조정: 추론 시 max_length를 150 이상(예: 200~256)으로 확장하여 충분한 생성 길이를 보장합니다.
...
inference.ipynb를 실행하여 실시간으로 외교부 블로그 최신 글 5건을 자동 수집 및 요약한 결과입니다.
...
데이터 준비: Google Drive에 data.csv 업로드 또는 코드 내 HuggingFace 다운로드 실행

학습: training.ipynb 실행 (모델 학습 및 Drive 저장)

평가: evaluation.ipynb 실행 (Test Set 로드 및 ROUGE 평가)

추론: inference.ipynb 실행 (자동으로 외교부 최신 소식을 가져와 요약합니다)
